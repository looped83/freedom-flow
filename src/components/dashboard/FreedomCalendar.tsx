...
export function FreedomCalendar({ freeDaysPerMonth }: FreedomCalendarProps) {
  ...
  const filledCount = Math.round(freeDaysPerMonth);

  return (
    <section className="bg-surface-1 rounded-2xl p-5" aria-labelledby="freedom-cal-title">
      <h2 id="freedom-cal-title" className="text-xs text-white/65 font-medium uppercase tracking-wider mb-3">
        Freedom Calendar
      </h2>
      ...
      <div
        role="img"
        aria-label={`${filledCount} von 30 Tagen pro Monat durch Dividenden finanziert`}
      ...
      >
        {Array.from({ length: 30 }, (_, i) => (
          <div
            key={i}
            className={`aspect-square rounded-sm ${i < filledCount ? 'bg-accent' : 'bg-white/10'}`}
          />
        ))}
      </div>
      <p className="text-xs text-white/55 mt-2">Jedes Kästchen = 1 Tag</p>
    </section>
  );
}
